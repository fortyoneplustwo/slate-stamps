...
A [plugin for Slate](https://docs.slatejs.org/concepts/08-plugins) that augments your text editor with auto-insertion of inline elements which react sanely to text editing actions.

## Features

- Add custom styling to stamps.
- Stamps can store arbitrary data.
...
A stamped line is represented as an `Element` node with these additional properties: `type`, `label`, and `value`. You must tell Slate how to render elements of this type. The augmented editor provides two helpful properties for exactly this purpose:
...
2. The new stamped element receives fresh data generated by a fresh call to `onStampInsert`.
3. This new element is then wrapped in a container matching the type of its original parent.
